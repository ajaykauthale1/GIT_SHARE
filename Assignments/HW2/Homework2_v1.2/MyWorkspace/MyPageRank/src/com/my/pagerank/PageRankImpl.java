...
public class PageRankImpl implements PageRankInterface {
	...
	public Map<String, Page> calculatePageRank(Map<String, Page> pageMap) {
		...
		/*
		 * replace while for running without checking convergence 
		 * also uncomment break-point at line 96 so that 100 iterations will run 
		 * 
		 * while (true) {*/
		while (!isConverged(pageMap)) {
			...
			/*
			 * uncomment for for running 100 iterations 
			 * 
	...
	@Override
	public Set<Page> getNodesWithFinalPRLessThanInitial(Map<String, Page> pageMap) {
		Set<Page> nodeSet = new HashSet<Page>();
		Double uniformPR = new Double(1.0 / PageRankConstants.N);

		// Iterate over pageMap
		Iterator<String> it = pageMap.keySet().iterator();
		while (it.hasNext()) {
			Page p = pageMap.get(it.next());
			// check if page has less page rank than uniform page rank
			if (p.getPageRank() < uniformPR) {
				nodeSet.add(p);
			}
		}
