...
;                                      DATA DEFINITIONS
;=========================================================================================
(define-struct node (posn mouse-posn parent-posn selected? hasparent?))
;; A Node is a (make-node Posn Posn Posn Boolean Boolean)
;; INTERPRETATION:
...
;; hasparent? - represents whether the node has any root node (parent) present or not
...
(define node-1-selected
  (make-node
   (make-posn 250 10) (make-posn 0 0) (make-posn 0 0) true false))
(define node-1-after-drag
  (make-node
   (make-posn 250 40) (make-posn 0 0) (make-posn 0 0) true false))
(define node-2-selected
  (make-node
   (make-posn 280 40) (make-posn 0 0) (make-posn 250 10) true true))
(define node-3-selected
  (make-node
   (make-posn 310 40) (make-posn 0 0) (make-posn 250 10) true true))
(define node-1-unselected
  (make-node
   (make-posn 250 10) (make-posn 0 0) (make-posn 0 0) false false))
(define node-2-unselected
  (make-node
   (make-posn 280 40) (make-posn 0 0) (make-posn 250 10) false true))
(define node-3-unselected
  (make-node
   (make-posn 310 40) (make-posn 0 0) (make-posn 250 10) false true))
...
(define world-with-tree-at-center
  (make-world
   (list (make-tree
          (make-node (make-posn 250 10) (make-posn 0 0) (make-posn 0 0) false true)
          empty))))
...
(define (world->scene ws)
  (display-all-branches (world-trees ws) (display-all-trees (world-trees ws))))

;; display-all-trees : ListOfTrees -> Image
;; GIVEN: a ListOfTree
;; RETURNS: the image of all trees drawn on the canvas
;; EXAMPLES:
#| (display-all-trees (list (make-tree
       (make-node
         (make-posn 250 10) (make-posn 0 0) (make-posn 0 0) true false) empty))
= (place-image NODE-CIRCLE 250 10 MT-SCENE)|#

;; STRATEGY: using template of ListOfTree on trees
(define (display-all-trees trees)
  (cond
    [(empty? trees) MT-SCENE]
    [else
     (place-images
      (get-list-of-node-images (descendants (first trees)))
      (get-list-of-posn (descendants (first trees)))
...
(define (get-list-of-node-images nodes)
  (foldr
   append empty
   (map
    ;; Node -> ListOfImage
    ...
    (lambda (node) (list (get-node-image node)))
    nodes)))
...
(scene+line
 (scene+line
  (scene+line
   (scene+line MT-SCENE 280 40 250 10 "black") 310 40 250 10 "black")
  280 40 250 10 "black")
 310 40 250 10 "black")|#
...
(begin-for-test
  ...
  (check-equal? (place-all-nodes-branches
                ...
                (scene+line
                 (scene+line MT-SCENE 280 40 250 10 "black")
                 310 40 250 10 "black"))
   ...
   (cons
    (make-tree
     (make-node
      ...
      (make-posn 0 0) (make-posn 0 0) false true) empty)
    (world-trees ws))))
...
(define (tree-after-mouse-event tree mx my me)
  (cond
    ...
    [(mouse=? me BUTTON-UP) (unselect-all-nodes-from-tree tree)]
...
;; selected? flag true with cursoe distance updated
;; EXAMPLES:
...
#| (select-node-from-sons (tree-sons tree-1-unselected) 250 10) =
 ...
 (make-tree (make-node (make-posn 280 40)
                       (make-posn -280 -40) (make-posn 250 10) #true #true) '())
 (make-tree (make-node (make-posn 310 40)
                       (make-posn 0 0) (make-posn 250 10) #false #true) '()))|#
...
#| (root-after-select node-1-unselected 50 50) =
                (make-node (node-posn node-1-unselected)
                           ...
                           true
                           (node-hasparent? node-1-unselected)))
|#
...
(define (root-after-select root mx my)
  (make-node (node-posn root)
             (make-posn
              (- (posn-x (node-posn root)) mx)
              (- (posn-y (node-posn root)) my))
             (node-parent-posn root)
             true
             (node-hasparent? root)))

;; unselect-all-nodes-from-tree : Tree -> Tree
;; GIVEN: a Tree
...
;; STRATEGY: using template of Tree on tree
(define (unselect-all-nodes-from-tree tree)
  (make-tree
       (root-after-unselect (tree-root tree))
       (unselect-node-from-sons (tree-sons tree))))

;; unselect-node-from-sons : ListOfTree -> ListOfTree
;; GIVEN: a ListOfTree
...
;; STRATEGY: using HOF foldr, followed by HOF map on sons
(define (unselect-node-from-sons sons)
  (foldr cons empty
   (map
   ;; Tree -> Tree
   ;; GIVEN: a Tree
   ...
   (lambda (son) (unselect-all-nodes-from-tree son))
   sons)))

;; root-after-unselect : Node -> Node
;; GIVEN: a Node
...
(define (root-after-unselect root)
  (make-node (node-posn root)
             (make-posn 0 0)
             (node-parent-posn root)
             false
...
(define (drag-son son root-posn)
  (make-tree
   ...
   (drag-all-sons (tree-sons son) root-posn)))
                ...
                (list
                 (make-tree
                  (make-node (make-posn 280 40)
                             (make-posn 0 0) (make-posn 250 10) #true #true) '())
                 (make-tree
