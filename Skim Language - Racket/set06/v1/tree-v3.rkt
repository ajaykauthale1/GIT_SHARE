...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname tree-v3) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
...
;                                      DATA DEFINITIONS
;=========================================================================================
(define-struct node (posn mouse-posn parent-posn selected? hasparent? deleted? dragselect?))
;; A Node is a (make-node Posn Posn Posn Boolean Boolean Boolean Boolean)
;; INTERPRETATION:
...
(define node-0-unselected
  (make-node
   (make-posn 100 100) (make-posn 0 0) (make-posn 0 0) false false false false))
(define node-0-selected
  (make-node
   (make-posn 100 100) (make-posn 0 0) (make-posn 0 0) true false false false))
(define node-selected
  (make-node
   (make-posn 150 150) (make-posn 0 0) (make-posn 0 0) true false false false))

(define node-1-selected
  (make-node
   (make-posn 250 10) (make-posn 0 0) (make-posn 0 0) true false false false))

(define node-1-after-drag
  (make-node
   (make-posn 250 40) (make-posn 0 0) (make-posn 0 0) true false false false))

(define node-2-selected
  (make-node
   (make-posn 280 40) (make-posn 0 0) (make-posn 250 10) true false false false))
(define node-3-selected
  (make-node
   (make-posn 310 40) (make-posn 0 0) (make-posn 250 10) true false false false))
(define node-1-unselected
  (make-node
   (make-posn 250 10) (make-posn 0 0) (make-posn 0 0) false false false false))
(define node-2-unselected
  (make-node
   (make-posn 280 40) (make-posn 0 0) (make-posn 250 10) false true false false))

(define node-3-unselected
  (make-node
   (make-posn 310 40) (make-posn 0 0) (make-posn 250 10) false false false false))
...
(define tree-1-root-selected
  (make-tree node-1-selected
             (list (make-tree node-3-unselected empty) (make-tree node-2-unselected empty))))
...
#;(begin-for-test
  (check-equal? (world-after-key-event (make-world (list )) "t")
                ...
                "It should return a new green outline circle at (250 10) on the empty scene")
                 ...
                 (list
                  ...
                  (make-tree
                   (make-node
                    ...
                    (make-posn 0 0) #false #false #false) '())))
                "A new circle should appear on scene at (250,10) and old one should maintain. ")
  ...
  ;                   (make-posn 0 0) (make-posn -10 10) #false #false) '())))
  
  ; Test not finished here. Because actually circle is not deleted. It is just move outside of scene. 
  ) 
   ...
   (cons
    (make-tree
     (make-node
      ...
      (make-posn 0 0) (make-posn 0 0) false false false false) empty)
    (world-trees ws))))
  ...
  (check-equal? (add-son-to-selected-root  tree-0-unselected)
                (make-tree
                 (make-node
                  (make-posn 100 100) (make-posn 0 0)
                  (make-posn 0 0) #false #false #false) '())
                "Because tree-0-unselected is unselected and the sons is empty, so just remain. "))
...
(list
 ...
 (make-tree
  ...
  (list
   (make-tree
    (make-node (make-posn 250 40) (make-posn 0 0) (make-posn 0 0) #true #false #false) '()))))
|#
  ...
  (if (empty? sons)
      (cons (make-tree
             (make-node (make-posn (posn-x posn) (+ NODE-DIST (posn-y posn)))
                        (make-posn -1 -1)
                        posn false true false false) empty) sons)
      (cons (make-tree
             (make-node (make-posn
  ...
  (check-equal? (add-chidren-to-right (list tree-and-his-son) (make-posn 250 10))
                (cons (make-tree
                       (make-node (make-posn
                                   ...
                                   (+ NODE-DIST 10))
                                  (make-posn -1 -1)
                                  (make-posn 250 10) false true false false) empty)
...
;; EXAMPLES:
#|
(delete-left-node-from-sons (list (make-tree
       (make-node
       (make-posn 100 40) (make-posn 0 0) (make-posn 250 10) false false false) empty))) =
(list (make-tree
(make-node (make-posn -100 -100) (make-posn 0 0) (make-posn -10 10) #false #false #false) '()))
|#
...
#;(begin-for-test
  (check-equal? (world-after-mouse-event (make-world (list tree-0-unselected)) 100 100  BUTTON-DOWN)
 ...
 (check-equal? (world-after-mouse-event (make-world (list tree-0-selected)) 100 100  BUTTON-UP)
                (make-world (list (make-tree (make-node
                                              (make-posn 100 100)
                                              (make-posn 100 100)
                                              (make-posn 0 0)
...
; (define node-0-selected
;  (make-node
;   (make-posn 100 100) (make-posn 0 0) (make-posn 0 0) true false))
;(define node-selected
;  (make-node
;   (make-posn 150 150) (make-posn 0 0) (make-posn 0 0) true false))

;(define drag-before-and-afteer
;  (make-tree (list (make-tree node-0-selected empty) (make-tree node-selected empty))) )
...
;; 
(define (drag-select-sons sons mx my)
  (foldr cons empty
         (map
          ;; Tree -> Tree
          ;; GIVEN: a Tree
          ;; RETURNS: tree after the drag event
          (lambda (son) (drag-select-son son mx my))
          sons)))

;;
(define (drag-select-son son mx my)
  (make-tree
   (son-after-drag-select (tree-root son) mx my)
   (drag-select-sons (tree-sons son) mx my)))

(define (son-after-drag-select root mx my)
  (make-node (node-posn root)
             (make-posn mx my) (node-parent-posn root)
             (node-selected? root) (node-hasparent? root)
...
(define (root-after-select root mx my)
  (make-node (node-posn root)
             (make-posn mx my) (node-parent-posn root)
             true (node-hasparent? root) (node-deleted? root) (node-dragselect? root)))
...
(define (root-after-unselect root mx my)
  (make-node
   ...
   (node-parent-posn root) false (node-hasparent? root) (node-deleted? root) (node-dragselect? root)))
...
(define (drag-node-from-tree tree mx my)
  (if (or (node-selected? (tree-root tree)) (node-dragselect? (tree-root tree)))
      (make-tree
       (root-after-drag (tree-root tree) mx my)
       (drag-node-from-sons
        (tree-sons tree)
        (posn-x (node-posn (root-after-drag (tree-root tree) mx my)))
        (posn-y (node-posn (root-after-drag (tree-root tree) mx my)))))
      (make-tree
       (tree-root tree)
...
#;(begin-for-test
  ...
  (check-equal? (drag-node-from-tree tree-0-unselected 20 20)
                (make-tree
                 (make-node (make-posn 100 100) (make-posn 0 0)
                            (make-posn 0 0) #false #false #false) '())
                "Because the node is unselected, thus it should not move which maitain at (100,100)"))
                 ...
                 (list
                  (make-tree
                   (make-node (make-posn 500 50) (make-posn 0 0)
                              (make-posn 250 10) #true #false #false) '())))
                "When drag root node-1-selected, his son node-1-after-drag should move together.
                  The new position of root should be (500, 20), and son should be (500, 50)"))
