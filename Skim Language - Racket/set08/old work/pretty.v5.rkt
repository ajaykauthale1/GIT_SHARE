     ...
     (local
       ...
       (if (= lo hi)
           ...
           (if (<= (+ (string-length exp-str) (string-length pre)) w)
               ...
               sub-exp-str)))]
    [(number? expr)
     (list (string-append pre (get-exp expr)))]))
...
;; get-sum-exps : LOExpr NonNegInt NonNegInt -> String
...
;; EXAMPLES:
#|
(get-sum-exps (list (make-sum-exp (list 1 2)) (make-diff-exp (list 4 5))) 1 2) =
"(+ (+ 1 2) (- 4 5))"
|#
...
;; EXAMPLES:
#|
(get-diff-exps (list (make-sum-exp (list 1 2)) (make-diff-exp (list 4 5))) 1 2) =
"(- (+ 1 2) (- 4 5))"
|#
