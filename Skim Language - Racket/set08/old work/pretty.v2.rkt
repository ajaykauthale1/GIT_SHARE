...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname pretty.v2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; get-exp : Expr -> String
(define (get-exp expr lst)
  (cond
    [(number? expr) (string-append " " (number->string expr))]
    [(sum-exp? expr)
     (get-sum-exps (sum-exp-exprs expr) 1 (length (sum-exp-exprs expr)) lst)]
    #;[(diff-exp? expr)
...
;; get-sum-exp : Expr NonNegInt NonNegInt -> ListOfString
(define (get-sum-exp expr lo hi lst)
  (cond
    [(= 1 lo)
     (list (string-append (last lst) " (+" (get-exp expr lst)))]
    [(= hi lo)
     (list (string-append (last lst) (get-exp expr lst) ")"))]
    [else
     (list (string-append (last lst) (get-exp expr lst)))]))

;; get-sum-exps : LOExpr -> ListOfString
(define (get-sum-exps exprs lo hi lst)
  (cond
    [(empty? exprs) empty]
    [else
     (local
       ((define first-exp (get-sum-exp (first exprs) lo hi lst))
        (define rest-exp (get-sum-exps (rest exprs) (+ lo 1) hi first-exp)))
       (list
        (last
         (append
          first-exp
...
;;
(define (out-of-room? lst expr wd)
  (cond
    [(and (< (last lst) wd) (> (get-exp expr lst) wd))
     (append lst (get-exp expr lst))]
    [else
     (list (string-append (last lst) " (+" (get-exp expr lst)))]))
