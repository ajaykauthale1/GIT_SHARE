...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname robot.v1) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; TESTS:
(begin-for-test
  (check-equal? (path '(1 2) '(2 1) '()) (list '("ne" 1)))
  (check-equal? (path '(0 3) '(3 0) '()) (list '("ne" 3)))
  (check-equal? (path '(1 2) '(3 2) '()) (list '("ne" 1) '("se" 1)))
  (check-false (path '(1 2) '(7 7) '())))
...
;; get-plan :
;;      Direction Direction PosInt Position Position ListOfPosition ListOfPosition
;;      ListOfPosition -> Plan
;; GIVEN:
...
;;          7. A list of blocks which caused robot to change it's direction
;;          8. A list of blocks previously traversed
;; RETURNS: the plan of moves for the next move
;; EXAMPLES:

;; STRATEGY:
(define (get-plan pre-dir cur-dir prev-step startpos endpos lop chnglop prevblocks)
  (local
    ...
    (if (next-pos-blocked? lop nextpos)
        (change-plan pre-dir cur-dir prev-step startpos endpos nextpos lop chnglop
                     prevblocks)
        (if (equal? unreachable? true)
            (list INVALID-MOVE)
            (if (equal? nextpos endpos)
                ...
                (if (equal? cur-dir pre-dir)
                    (append (list (list cur-dir (+ 1 prev-step)))
                            (get-plan
                             cur-dir
                             (next-dir-to-move nextpos endpos)
                             (+ 1 prev-step)
                             nextpos endpos lop chnglop
                             (append (list startpos) prevblocks)))
...
;;          8. A list of blocks which caused robot to change it's direction
...
(define (change-plan pre-dir cur-dir prev-step startpos endpos nextpos
                     lop chnglop prevblocks)
  (local
    ((define newdir (change-dir cur-dir))
     (define oldchnglop chnglop))
    (if (next-pos-blocked? prevblocks nextpos)
        (list INVALID-MOVE)
        (if (not (set-equal? lop oldchnglop))
            (local
              ((define newchnglop (append (list nextpos) chnglop)))
              (get-plan pre-dir newdir prev-step startpos endpos
                        lop newchnglop prevblocks))
            (list INVALID-MOVE)))))
...
(define (next-pos-blocked? lop nextpos)
  (ormap
   ;; Position -> Boolean
   ;; GIVEN: A Position
   ;; RETURNS: true iff the nextpos equal to pos
   (lambda (pos) (equal? pos nextpos))
   lop))
...
;; GIVEN:   a direction
;; RETURNS: new direction in clockwise side of the old direction
;; EXAMPLES: see tests below

;; STRATEGY: using cases on olddir
(define (change-dir olddir)
  (cond
    [(equal? olddir NE) SE]
    [(equal? olddir SE) SW]
    [(equal? olddir SW) NW]
...
;; TESTS:
(begin-for-test
  (check-equal? (change-dir NE) SE)
  (check-equal? (change-dir SE) SW)
  (check-equal? (change-dir SW) NW)
  (check-equal? (change-dir NW) NE))
