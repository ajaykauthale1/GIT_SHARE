...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname screensaver-4) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
...
;; initial world state
(define initial-worldstate (make-WorldState empty true empty))
;; world state with added rectangle
(define initial-worldstate-with-rect (make-WorldState (cons rect-at-canvas-center empty) true empty))
(define initial-worldstate-with-rect-selected (make-WorldState (cons rect-at-canvas-center-selected empty) true empty))
;; unpaused initial world state
(define unpaused-initial-worldstate (make-WorldState empty false empty))
;; world after tick
(define worldstate-after-tick (make-WorldState empty true empty))
;; worlds where rectangles are selected
(define worldstate-with-rect1 (make-WorldState (cons rect1-selected empty) true empty))
(define worldState-with-empy-rects (make-WorldState empty true empty))
;; worlds after various arrow key events
(define worldstate-after-key-up (make-WorldState (cons rect1-selected-speed-up empty) true empty))
(define worldstate-after-key-down (make-WorldState (cons rect1-selected-speed-down empty) true empty))
(define worldstate-after-key-right (make-WorldState (cons rect1-selected-speed-right empty) true empty))
(define worldstate-after-key-left (make-WorldState (cons rect1-selected-speed-left empty) true empty))
;; worlds after button events
(define worldstate-pen-up (make-WorldState (cons rect2-selected empty) true empty))
(define worldstate-pen-down (make-WorldState (cons rect2-pendown empty) true empty))
...
(define (initial-world input)
  (make-WorldState empty true empty))
...
(begin-for-test
  ...
  ;; test rectangles whose pen is down
  (check-equal? (get-track-list (cons rect2-pendown empty) empty) (cons (make-Point RECT2-CENTER-X RECT2-CENTER-Y) empty))
  ;; test rectangles whose pen is up
  (check-equal? (get-track-list (cons rect1 empty) empty) (cons (make-Point -10 -10) empty)))
;=================================================================================================================
...
(begin-for-test
  ...
  (check-equal? (world-after-key-event worldstate-pen-up "d") worldstate-pen-down
                "world-after-key-event should return the worldstate with selected rectangles pen down")
   (check-equal? (world-after-key-event worldstate-pen-down "u") worldstate-pen-up
                 "world-after-key-event should return the worldstate with selected rectangles pen up"))
;=================================================================================================================
;**************************************************** Check ***********************************************88
...
(define (get-list-of-rects rects)
  (cond
    ...
    [else (cons (if (rect-selected? (first rects))
                    (first rects)
                    (make-Rectangle (rect-x (first rects)) (rect-y (first rects))
                                    (rect-vx (first rects)) (rect-vy (first rects))
                                    false ZERO ZERO (rect-pen-down? (first rects))))
...
;; EXAMPLES:
;; (world-paused? unpaused-initial-worldstate) = true
